		...
		if (!world.isClientSide() && entityLiving instanceof ServerPlayer entity) {
			...
			if (true) {
				ItemStack stack = ProjectileWeaponItem.getHeldProjectile(entity,
						e -> e.getItem() == AetheriumresourcesModItems.METAL_PIPE_ITEM.get());
				if (stack == ItemStack.EMPTY) {
					for (int i = 0; i < entity.getInventory().items.size(); i++) {
						ItemStack teststack = entity.getInventory().items.get(i);
						if (teststack != null && teststack.getItem() == AetheriumresourcesModItems.METAL_PIPE_ITEM.get()) {
							stack = teststack;
							break;
						}
					}
				...
				if (entity.getAbilities().instabuild || stack != ItemStack.EMPTY) {
					...
					if (entity.getAbilities().instabuild) {
						...
						if (new ItemStack(AetheriumresourcesModItems.METAL_PIPE_ITEM.get()).isDamageableItem()) {
